...
function indexOfInternal(distance) {
    ...
    var index = -1;
    var block = null;
    var blockGap = this.getGap() * this.block_size;

    // Find the block that contains distance and the index of its
    // first element
    var blockIndex = -1,
        blockTableLength = this.block_table.length;
    
    for(; ++blockIndex < blockTableLength;) {
        block = this.block_table[blockIndex];
        var blockDistance = blockGap + (block ?
            block.sizesSum + (block.defaultCount * this._defaultSize) :
            this.block_size * this._defaultSize);
    ...
    if(block) {
        ...
        var sizes = block.sizes;
        for(var i = 0; i < this.block_size - 1; i++) {
            var size = sizes[i];
            curDistance += this.getGap() + (size !== size ? this._defaultSize : size);
        ...
        return index + Math.floor(Number(distance - curDistance) / Number(this._defaultSize + this.getGap()));
    }
}
...
function toString() {
    return "SparseArray{" +
        ...
        " this.removes=" + (this.removes ? this.removes.length : 0) +
        " this.inserts=" + (this.inserts ? this.inserts.length : 0) +
        "}";
}
